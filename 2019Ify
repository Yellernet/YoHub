...
local function EGVWZV_fake_script() -- PlayerListContainer.LocalScript 
    ...
    local function update(plr)
        ...
        local function updateTeamColor()
            if plr.Team then
                ...
                clone.BGFrame.BackgroundColor3 = Color3.fromRGB(31, 31, 31) -- Default to gray if no team
            end
        end
        ...
        if leaderstats then
            for _, stat in ipairs(leaderstats:GetChildren()) do
                stat:GetPropertyChangedSignal("Value"):Connect(function()
                    update(plr) -- Update the specific player's UI on stat change
                end)
            end
        end
    end
    ...
    local function updateAllPlayers()
        local playerEntries = {}
        for _, plr in ipairs(Players:GetPlayers()) do
            update(plr)  -- Update each player's UI individually
...
-- Handle visibility toggle for the player list
